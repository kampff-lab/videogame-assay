...
namespace Bonsai.Videogame
{
    static class VertexHelper
    {
        ...
        public static int UpdateIndexBuffer<TIndex>(int indexBuffer, TIndex[] buffer, BufferUsageHint usage)
            where TIndex : struct
        {
            var bufferSize = buffer.Length * BlittableValueType<TIndex>.Stride;
